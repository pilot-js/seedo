...
const syncAndSeed = () => {
	return conn.sync({ force: true })
		...
		.then(([users, challenges, userchallenges, solutions, images]) => {
			...
			return Promise.all([
				userchallenges.find(chal => chal.css.includes('circle')).update({ userId: 2, challengeId: 1 }),
				userchallenges.find(chal => chal.css.includes('square')).update({ userId: 3, challengeId: 2 }),
