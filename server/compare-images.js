...
const compareImages = (userchallengePath, challengeImg) => {
  const img1 = fs.createReadStream(challengeImg).pipe(new PNG()).on('parsed', doneReading);
  const img2 = fs.createReadStream(userchallengePath).pipe(new PNG()).on('parsed', doneReading);
  const filesRead = 0;
