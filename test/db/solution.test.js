...
describe('testing solution model', () => {
    beforeAll(async () => {
        ...
        await Solution.create({html: 'html TEXT', css: 'css TEXT', js: 'js TEXT', challengeId: challenge1.id});
    })
    afterAll(async () => {
        await db.close();
    })
    ...
    it('can create a solution for a challenge', async () => {
        const challenge1 = await Challenge.findOne({where: {name: 'challenge1'}})
        await Solution.create({html: 'html TEXT1', css: 'css TEXT', js: 'js TEXT', challengeId: challenge1.id});
